...
import Display, { type DisplayHandle } from './components/Display'
...
export default function Home() {
  ...
  const displayRef = useRef<DisplayHandle | null>(null)
  ...
  const showTab = useCallback((next: Tab) => {
    setTab(next)
    buzz(8)
    // The display canvas reports zero width while its tab is hidden and
    // renders blank forever if never resized after becoming visible again.
    if (next === 'rec') requestAnimationFrame(() => displayRef.current?.resize())
  }, [])
